...
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        ...
        <div class=" bg-white max-w-md rounded overflow-hidden shadow-xl p-5">
            ...
            <form autocomplete="off" class="space-y-4" action="{{ route('mahasiswas.update', $mahasiswa) }}" method="POST">
                @csrf
                @method('PATCH')
                <div class="rounded-md shadow-sm -space-y-px">
                    <div class="grid gap-6">
                        <div class="col-span-12">
                            ...
                            <input type="text" value="{{ $mahasiswa->nama }}" name="nama" id="first_name" autocomplete="given-name"
                        ...
                        <div class="col-span-12">
                            ...
                            <select name="gender" id="email_address" 
                                class="mt-1 px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                <option value="male" {{ $mahasiswa->gender == 'male' ? 'selected' : '' }}>Male</option>
                                <option value="female" {{ $mahasiswa->gender == 'female' ? 'selected' : '' }}>Female</option>
                            </select>
                        </div>

                        <div class="col-span-12">
                            <label for="email_address" class="block text-sm font-medium text-gray-700">Umur</label>
                            <input type="number" value="{{ $mahasiswa->umur }}" name="umur" id="email_address" autocomplete="email"
                                class="mt-1 px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>

                        <div class="col-span-12">
                            <label for="email_address" class="block text-sm font-medium text-gray-700">Alamat</label>
                            <textarea name="alamat" id="email_address" class="mt-1 px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ $mahasiswa->alamat }}</textarea>
                        </div>
                    </div>
                </div>
